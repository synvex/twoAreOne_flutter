...
class InterestedViewModel extends ChangeNotifier {
  ...
  /// Same shared-banner hookup used by FavouriteViewModel - see the
  /// comment on FavouriteViewModel._stats for why this goes through
  /// navigatorKey instead of a constructor-injected BuildContext.
  ...
  Future<void> blockSelectedUser() async {
    ...
    try {
      ...
      _stats?.userBlocked(
        wasInterested: _activeTab == InterestedTab.yourInterested,
      );
    } on InterestedApiException catch (e) {
  ...
  Future<void> removeFromYourInterested() async {
    ...
    try {
      ...
      _showBottomSheet = false;
      _removeFromCurrentList(item.id);
      // Only "Your Interested" feeds the Home banner's INTERESTED count.
      _stats?.setInterested(false);
    } on InterestedApiException catch (e) {
      _errorMessage = e.message;
      _removeLoading = false;
  ...
  Future<void> removeFromInterestedYou() async {
    ...
    try {
      ...
      _showBottomSheet = false;
      _removeFromCurrentList(item.id);
      // Safety net: this tab isn't expected to move the banner's
      // INTERESTED count (that's driven by "Your Interested"), but calling
      // reconcile() here costs one cheap network read and guarantees the
      // banner can never silently drift if that assumption is ever wrong.
      _stats?.reconcile();
    } on InterestedApiException catch (e) {
      _errorMessage = e.message;
      _removeLoading = false;
