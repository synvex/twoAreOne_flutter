...
class _ChangePhoneScreenState extends State<ChangePhoneScreen> {
  ...
  void _goToSettings() => Navigator.of(context).pushReplacementNamed(AppRoutes.settingScreen);
  ...
  Future<void> _onContinue() async {
    ...
    try {
      ...
      if (confirmation != null) {
        Navigator.of(context).pushNamed(AppRoutes.changeOtpScreen, arguments: {
          'phone': phone,
          'endTime': DateTime.now().add(const Duration(seconds: 60)),
        });
      }
    } on ApiError catch (e) {
      setState(() => _error = e.message.isNotEmpty ? e.message : 'This Number is already linked to another account. Please use a different number');
    } catch (_) {
      if (mounted) await CustomAlert.showMessage(context, title: 'Error', message: 'Failed to send OTP. Please try again.');
    } finally {
        ...
        body: SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                BackButtonHeader(title: 'Add New Phone', noIcon: true, onPress: _goToSettings),
                const SizedBox(height: 16),
                Text('Add Your New Phone Number', style:
                GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w700)),
                const SizedBox(height: 8),
                Text('Enter the new phone number you want to use. We will send an OTP for verification.',
                    style: GoogleFonts.poppins( fontSize: 13, color: AppColors.grey2)),
                const SizedBox(height: 16),
                CustomTextField(controller: _phone, hint: '+1 234 567 8900', keyboardType: TextInputType.phone, errorText: _error),
                const SizedBox(height: 20),
                CustomButton(title: 'Send OTP', loading: _loading, onPress: _onContinue),
              ],
            ),
          ),
        ),
