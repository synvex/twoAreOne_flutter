...
class _FavouriteScreenState extends State<FavouriteScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<UserStatsViewModel>().addListener(_onExternalChange);
      }
    });
  }

  void _onExternalChange() {
    if (mounted) _getData(page: 1, refresh: true);
  }
  ...
  void dispose() {
    ...
    context.read<UserStatsViewModel>().removeListener(_onExternalChange);
    super.dispose();
  }
  ...
  Future<void> _onUnfavoritePress(FilterMatchModel item) async {
    ...
    try {
      ...
      // ✅ Keep the Home banner's FAVOURITES count correct instantly.
      context.read<UserStatsViewModel>().setFavorite(false);
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _onBlockPress(FilterMatchModel item) async {
    ...
    try {
      ...
      if (success) {
        ...
        // ✅ This list only ever contains favourited users, so blocking one
        // here always removes a FAVOURITES entry too.
        context.read<UserStatsViewModel>().userBlocked(wasFavorite: true);
      } else {
