...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) context.read<UserStatsViewModel>().loadUserInfo();
    });
  ...
  void _handleFavorite(FilterMatchModel user) async {
    ...
    if (success && mounted) {
      ...
      final wasTrue = index != -1 ? _users[index].isFavorite : user.isFavorite;
      setState(() {
        if (index != -1) {
          _users[index] = _users[index].copyWith(isFavorite: !wasTrue);
        }
      });
      context.read<UserStatsViewModel>().setFavorite(!wasTrue);
    }
  ...
  void _handleInterest(FilterMatchModel user) async {
    ...
    if (success && mounted) {
      final index = _users.indexWhere((u) => u.id == user.id);
      final wasTrue = index != -1 ? _users[index].isInterested : user.isInterested;
      setState(() {
        if (index != -1) {
          _users[index] = _users[index].copyWith(isInterested: !wasTrue);
        }
      });
      context.read<UserStatsViewModel>().setInterested(!wasTrue);
    }
  ...
  Widget _buildActionBanner(UserStatsViewModel stats) {
    return Container(
      decoration: BoxDecoration(
                      ...
                      Stack(
                        ...
                        children: [
                          ...
                          stats.profileImageUrl != null &&
                              stats.profileImageUrl!.isNotEmpty
                              ? Container(
                            width: 58.w,
                            height: 58.h,
                            decoration: BoxDecoration(
                  ...
                  IntrinsicHeight(
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: _buildStat(
                            stats.interestedCount.toString(),
                            "INTERESTED",
                          ),
                        ),
  ...
  Widget _buildDrawer(UserStatsViewModel stats) {
    return Drawer(
      backgroundColor: Colors.white,
      child: Column(
        children: [
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.network(
                          ...
                          errorBuilder: (context, error, stackTrace) => Text(
                            stats.userName.toString().trim()[0].toUpperCase(),
                            style: TextStyle(
                              fontWeight: FontWeight.w500,
                              color: Colors.white,
                            ),
