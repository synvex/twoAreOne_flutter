// lib/ui/views/settings/settings_screen.dart
//
// Flutter port of RN's `Screens/AppScreens/SettingsScreen/index.js`
// (+ `styles.js`), rebuilt with a strict MVVM split:
//   - Model     -> data/models/settings_user_info.dart
//   - Service   -> data/services/settings_service.dart
//   - ViewModel -> ui/viewmodels/settings_view_model.dart  (all state/logic)
//   - View      -> this file                               (UI only)
//
// Navigation-out-of-scope note: PrivacyPolicy, TermsOfUse, ResetPassword,
// ChangeOtp and ChangeEmailOtp screens don't exist in this Flutter app
// yet (only this Settings screen was in scope). Navigating to them uses
// the route names in `core/settings_routes.dart`; until those screens are
// built and registered, tapping those rows shows a "Coming soon" toast
// (same pattern already used by ProfileScreen for its own unbuilt menu
// items) instead of crashing.
//
// Back-navigation note: RN dispatches `setRefreshProfile(!refresh)` before
// going back so ProfileScreen re-fetches on return. This app has no
// global store, so this screen instead pops with `Navigator.pop(true)` —
// the caller can await the push (exactly like it already does for
// `/edit_profile` in profile_screen.dart) and re-fetch its own data when
// the result is `true`.
...
import 'package:two_are_one/ui/views/Settings/widgets/dialog.dart';
import 'package:two_are_one/ui/views/Settings/widgets/editable_items.dart';
import 'package:two_are_one/ui/views/Settings/widgets/items.dart';
import 'package:two_are_one/ui/views/bottom_nav/profile_screen.dart';

import '../../../core/confirmation_dialogue.dart';
